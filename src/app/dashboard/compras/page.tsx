...
import { 
  ...
  Truck, CheckCircle2, Factory, ScrollText, Loader2
} from 'lucide-react';
...
// === Mock Data (Fallback until compras table is created) ===
...
export default function ControleCompras() {
  ...
  const fetchComprasData = async () => {
    setLoading(true);
    try {
      // Carregar fornecedores reais para o gráfico
      const forns = await api.getFornecedores();
      setFornecedores(forns);
      
      // TODO: When "compras" table is created, fetch here
      // For now, use mock compras but process CMED alerts from real medicamentos prices when available
  ...
  // Build chart data from real fornecedores
  const chartData = fornecedores.length > 0 
    ? fornecedores.slice(0, 5).map(f => ({
        supplier: f.razao_social.length > 18 ? f.razao_social.substring(0, 18) + '...' : f.razao_social,
        contractual: Math.max(f.lead_time_medio - 2, 3), // estimated contractual
    ...
    <div className="p-8 space-y-6 bg-slate-50/50 min-h-full pb-20 relative">
      ...
      {loading ? (
        <div className="p-16 text-center">
          ...
          <p className="text-sm text-slate-500 font-medium">Carregando dados de compras...</p>
        </div>
      ) : (
        <>
          {/* CHARTS LAYER */}
